...
def elegir_opcion_filtro(doc,f1):
	try:
		opciones = int(input('\nEscoja un número de la lista de {}: ("0" para terminar): '.format(f1)))
		if opcion not in range(len(doc[f1])+1):
			print ('Escoja elija un número de la lista')
...
def mostrar_restaurantes(key,url1,url2=''):
	...
	while len(doc['restaurants']) == 20 and num != 80:
		print('\n¿Quiere viendo restaurantes? ("s"/SI "n"/NO)')
		opcion = continuar()
